  ...
  return (
    <div className="space-y-6" data-testid="price-tracking-page">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-slate-900" data-testid="page-title">Fiyat Takip & Tarama</h1>
          <p className="text-sm text-slate-500">Rakip fiyat taraması, karşılaştırma ve otomatik fiyatlama</p>
        </div>
        ...
        <div className="bg-violet-50 border border-violet-200 rounded-xl p-4" data-testid="scan-progress">
          ...
          <div className="w-full bg-violet-200 rounded-full h-2">
            <div
              ...
              style={{ width: `${scanStatus.total ? (scanStatus.scanned / scanStatus.total * 100) : 0}%` }}
            />
          </div>
          {scanStatus.current_product && (
            <p className="text-xs text-violet-600 mt-1.5 truncate">Taranan: {scanStatus.current_product}</p>
          )}
        </div>
      )}
        ...
        <div className="bg-white rounded-xl border p-4 text-sm" data-testid="last-scan-info">
          <div className="flex items-center justify-between">
            ...
            <span className="font-medium">
              {new Date(dashboard.scan_status.completed_at).toLocaleDateString("tr-TR")}{" "}
              {new Date(dashboard.scan_status.completed_at).toLocaleTimeString("tr-TR", { hour: "2-digit", minute: "2-digit" })}
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border shadow-sm" data-testid="category-rules-section">
          <div className="flex items-center justify-between px-4 py-3 border-b">
            <h2 className="font-semibold text-sm text-slate-800 flex items-center gap-1.5">
              <Settings2 className="h-4 w-4" /> Kategori Fiyatlama Kuralları
            </h2>
            <Button size="sm" variant="outline" onClick={() => { setShowRuleDialog(true); fetchCategories(); }} data-testid="add-rule-btn">
              <Plus className="h-3.5 w-3.5 mr-1" /> Kural Ekle
            </Button>
          </div>
          <div className="p-3">
              ...
              <div className="space-y-2">
                {(dashboard?.category_rules || []).map(rule => (
                  <div key={rule.category_name} className={`flex items-center justify-between p-3 rounded-lg border ${rule.enabled ? "bg-white border-slate-200" : "bg-slate-50 border-slate-100 opacity-60"}`} data-testid={`rule-${rule.category_name}`}>
                    ...
                    <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-red-400 hover:text-red-600 hover:bg-red-50" onClick={() => deleteRule(rule.category_name)}>
        ...
        <div className="bg-white rounded-xl border shadow-sm" data-testid="recent-changes-section">
          <div className="px-4 py-3 border-b">
            <h2 className="font-semibold text-sm text-slate-800 flex items-center gap-1.5">
              <TrendingDown className="h-4 w-4" /> Son Fiyat Önerileri
            </h2>
          </div>
          <div className="p-3">
                ...
                {(dashboard?.recent_changes || []).map((ch, i) => (
                  <div key={i} className="p-3 bg-slate-50 rounded-lg text-sm border">
                    ...
                    <div className="flex items-center gap-2 mt-1.5 text-xs">
                      ...
                      <span className={`ml-auto px-1.5 py-0.5 rounded text-[10px] font-medium ${ch.applied ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
                        {ch.applied ? "Uygulandı" : "Bekliyor"}
                      </span>
                    </div>
                    <div className="text-[11px] text-slate-400 mt-1">{ch.reason}</div>
                    <div className="text-[10px] text-slate-400 mt-0.5">
                      {ch.changed_at ? new Date(ch.changed_at).toLocaleDateString("tr-TR") + " " + new Date(ch.changed_at).toLocaleTimeString("tr-TR", { hour: "2-digit", minute: "2-digit" }) : ""}
                    </div>
                  </div>
                ))}
              </div>
      ...
      <Dialog open={showRuleDialog} onOpenChange={setShowRuleDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Kategori Fiyatlama Kuralı</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 pt-2">
            <div>
              ...
              <select
                ...
                className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-violet-200 focus:border-violet-400 outline-none"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <div className="relative">
                  <Input
                    type="number"
                    value={newRule.profit_margin_pct}
                    onChange={e => setNewRule(r => ({ ...r, profit_margin_pct: parseFloat(e.target.value) || 0 }))}
                    className="pr-8"
                    data-testid="rule-margin-input"
                  />
              ...
              <div>
                ...
                <Input
                  type="number"
                  value={newRule.undercut_amount}
                  onChange={e => setNewRule(r => ({ ...r, undercut_amount: parseFloat(e.target.value) || 0 }))}
                  data-testid="rule-undercut-input"
                />
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => setNewRule(r => ({ ...r, enabled: !r.enabled }))}
                className={`w-10 h-5 rounded-full transition-colors ${newRule.enabled ? "bg-violet-600" : "bg-slate-300"}`}
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-700">
              <strong>Kural mantığı:</strong> Dip Fiyat = Alış Fiyatı × (1 + Kâr Marjı%). Sistem, en ucuz rakipten Kırma tutarı kadar düşük fiyat önerir ancak Dip Fiyat'ın altına inmez.
            </div>
            <div className="flex gap-2 justify-end pt-2">
              <Button variant="outline" onClick={() => setShowRuleDialog(false)}>İptal</Button>
              <Button onClick={saveRule} className="bg-violet-600 hover:bg-violet-700 text-white" data-testid="save-rule-btn">
